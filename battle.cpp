...
void display() //画面初期化
{
	...
	if (speed == 10) {
		...
		mvprintw(3, 10, "(S)speed  ×0%d", speed);
	}
	if (autoMode) {
			...
			if (autoMode) {//Autoモード
				...
				while (1) {//技選択
					...
					for (int i = 1; i <= 6; i++) {
						...
						mvprintw(22 + i, 8, "%d:%s(%s)", i, p.m[i - 1].move_name, p.m[i - 1].move_exp);
						attrset(COLOR_PAIR(0));
					}
					refresh();
		...
		if (p.HP <= 0) {
			...
			while (1) {
				...
				for (int i = 1; i <= 6; i++) {
					...
					mvprintw(22 + i, 8, "%d:%s(%s)", i, monster[mnumber].m[i - 1].move_name, monster[mnumber].m[i - 1].move_exp);
					attrset(COLOR_PAIR(0));
				}
				refresh();
