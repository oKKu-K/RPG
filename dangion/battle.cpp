			...
			if (autoMode) {//Autoモード
				...
				while (1) {//技選択
					...
					for (int i = 1; i <= 6; i++) {
						...
						mvprintw(22 + i, 8, "%d:%s(攻撃%d,DP消費%d)", i, p.m[i - 1].move_name, p.m[i - 1].att_point, p.m[i - 1].DP_cons);
						attrset(COLOR_PAIR(0));
					}
					refresh();
					...
					if (key == 's' && !autoMode) {//スピード変更
						...
						for (int i = 1; i <= 6; i++) {
							...
							mvprintw(22 + i, 8, "%d:%s(攻撃%d,DP%d)", i, p.m[i - 1].move_name,p.m[i-1].att_point,p.m[i-1].DP_cons);
							attrset(COLOR_PAIR(0));
						}
						display();
		...
		if (p.HP <= 0) {
			...
			while (1) {
				...
				for (int i = 1; i <= 6; i++) {
					...
					mvprintw(22 + i, 8, "%d:%s(攻撃%d,DP%d)", i, monster[mnumber].m[i - 1].move_name, monster[mnumber].m[i - 1].att_point, monster[mnumber].m[i - 1].DP_cons);
					attrset(COLOR_PAIR(0));
				}
				refresh();
